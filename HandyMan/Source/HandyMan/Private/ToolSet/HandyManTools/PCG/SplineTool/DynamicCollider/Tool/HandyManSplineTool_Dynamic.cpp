...
void USplineTool_Dynamic::SpawnActorInstance(const UDynamicSplineToolProperties* CurrentSettings)
{
	if (GetHandyManAPI())
	{
		...
		auto ClassToSpawn = GetHandyManAPI()->GetPCGActorClass(FName(this->ToolName.ToString()));
		if (auto SpawnedActor =  World->SpawnActor<APCG_DynamicSplineActor>(ClassToSpawn))
		{
			SpawnedActor->SetActorTransform(FTransform::Identity);
			TargetSplineActor = Cast<APCG_DynamicSplineActor>(SpawnedActor);
			TargetSplineActor->GetPCGComponent()->GenerateLocal(true);

			if (CurrentSettings)
			{
				TargetSplineActor->SetSplinePointType(CurrentSettings->SplineType);
				TargetSplineActor->Tags = Settings->ActorTags;
				TargetSplineActor->SetCollisionProfileByName(Settings->CollisionProfileName);
			}
		}
...
void USplineTool_Dynamic::Setup()
{	
	Super::Setup();

	
	Settings = NewObject<UDynamicSplineToolProperties>(this);
	Settings->RestoreProperties(this);
	AddToolPropertySource(Settings);

	Settings->TargetActor = SelectedActor;

	SetToolDisplayName(LOCTEXT("SplineToolName", "Rail Tool"));
	GetToolManager()->DisplayMessage(
		LOCTEXT("DrawSplineToolDescription", "Draw a grind rail for players to interact with."),
	...
	Settings->WatchProperty(Settings->ActorTags, [this](TArray<FName>)
	{
		if (TargetSplineActor)
		{
			TargetSplineActor->Tags = Settings->ActorTags;
	...
	Settings->WatchProperty(Settings->CollisionProfileName, [this](FName)
	{
		if (TargetSplineActor)
		{
			TargetSplineActor->SetCollisionProfileByName(Settings->CollisionProfileName);
	...
	Settings->WatchProperty(Settings->RandomizedSeed, [this](int32)
		{
			if (TargetSplineActor)
			{
				TargetSplineActor->GetPCGComponent()->Seed = Settings->RandomizedSeed;
			}
		});
	...
	Settings->WatchProperty(Settings->ColliderAdditiveScale, [this](FVector2D)
    {
    	if (TargetSplineActor)
    	{
    		TargetSplineActor->SetColliderAdditiveScale(Settings->ColliderAdditiveScale);
    	}
    });
...
void USplineTool_Dynamic::TransitionOutputMode()
{
...
void USplineTool_Dynamic::Shutdown(EToolShutdownType ShutdownType)
{
...
void USplineTool_Dynamic::GenerateAsset()
{
	using namespace HandyMan::SplineUtils;
...
void USplineTool_Dynamic::SetSplinePointsOnTargetActor()
{
	if (TargetSplineActor)
	{
...
void USplineTool_Dynamic::AddSplinePoint(const FVector3d& HitLocation, const FVector3d& HitNormal)
{
	using namespace HandyMan::SplineUtils;
...
FVector3d USplineTool_Dynamic::GetUpVectorToUse(const FVector3d& HitLocation, const FVector3d& HitNormal, int32 NumSplinePointsBeforehand)
{
...
bool USplineTool_Dynamic::Raycast(const FRay& WorldRay, FVector3d& HitLocationOut, FVector3d& HitNormalOut, double& HitTOut)
{
...
FInputRayHit USplineTool_Dynamic::IsHitByClick(const FInputDeviceRay& ClickPos)
{
...
void USplineTool_Dynamic::OnClicked(const FInputDeviceRay& ClickPos)
{
	using namespace HandyMan::SplineUtils;
...
FInputRayHit USplineTool_Dynamic::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
	/*FVector3d HitLocation, HitNormal;
	double HitT;
...
void USplineTool_Dynamic::OnClickPress(const FInputDeviceRay& PressPos)
{
	FVector3d HitLocation, HitNormal;
	double HitT;
...
void USplineTool_Dynamic::OnClickDrag(const FInputDeviceRay& DragPos)
{
	using namespace HandyMan::SplineUtils;
...
void USplineTool_Dynamic::OnClickRelease(const FInputDeviceRay& ReleasePos)
{
	OnClickDrag(ReleasePos);
	OnTerminateDragSequence();
}
void USplineTool_Dynamic::OnTerminateDragSequence()
{
	using namespace HandyMan::SplineUtils;
...
void USplineTool_Dynamic::OnTick(float DeltaTime)
{
	if (PlaneMechanic)
	{
...
void USplineTool_Dynamic::Render(IToolsContextRenderAPI* RenderAPI)
{
	using namespace HandyMan::SplineUtils;
...
void USplineTool_Dynamic::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
}

bool USplineTool_Dynamic::CanAccept() const
{
...
void USplineTool_Dynamic::FDynamicSplineToolChange::Apply(UObject* Object)
{
	USplineTool_Dynamic* Tool = Cast<USplineTool_Dynamic>(Object);
	if (!ensure(Tool))
	{
		return;
	}
...
void USplineTool_Dynamic::FDynamicSplineToolChange::Revert(UObject* Object)
{
	USplineTool_Dynamic* Tool = Cast<USplineTool_Dynamic>(Object);
	if (!ensure(Tool))
	{
